var Guide = $V.classes.VViewController.extend({
	...
	viewDidLoad: function(aView) {
		var self = this;
		this.player = $V.MovieView.alloc({ id: "GuidePlayer", autoPlay: false, controls: false, url: ["media/guide/hawker.mp4", "media/guide/hawker.ogg", "media/guide/hawker.webm"] });
//		this.player.setStyle('position', 'absolute');
	...
	play: function( video ) {
		switch (video) {
			case "welcome":
				this.player.load( ["media/guide/welcome.mp4", "media/guide/welcome.ogg", "media/guide/welcome.webm"] );
			break;
			case "hawker":
				this.player.load( ["media/guide/hawker.mp4", "media/guide/hawker.ogg", "media/guide/hawker.webm"] );
			break;
			default:
		}
